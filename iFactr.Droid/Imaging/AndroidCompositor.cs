...
namespace MonoCross.Utilities.ImageComposition
{
    ...
    public class AndroidCompositor : ICompositor
    {
        ...
        {
            if (!overwrite && iApp.File.Exists(saveLocation) || paths.Count == 0)
                return;

            var bytes = paths.Select(path =>
            {
                var b = iApp.File.Read(path, EncryptionMode.NoEncryption);
                if (b == null)
                {
            ...
            iApp.File.Save(saveLocation, image, EncryptionMode.NoEncryption);
        }
        ...
        public byte[] CreateCompositeImage(List<byte[]> imageBytes, string extension)
        {
            ...
            iApp.Log.Metric("ImageEngine icon creation", DateTime.UtcNow.Subtract(metric).TotalMilliseconds);
